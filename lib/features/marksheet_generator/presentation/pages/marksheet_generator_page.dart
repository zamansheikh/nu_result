...
class _MarksheetGeneratorPageState extends State<MarksheetGeneratorPage> {
  ...
  void initState() {
    ...
    _loadStudentInformation(); // Load saved student info
  }
    ...
    try {
      ...
      setState(() {
        ...
        _registrationController.text =
            prefs.getString('student_registration') ?? '';
        _sessionController.text = prefs.getString('student_session') ?? '';
        _collegeController.text = prefs.getString('student_college') ?? '';
      });
    } catch (e) {
  ...
  Future<void> _saveStudentInformation() async {
    try {
      ...
      await prefs.setString('student_session', _sessionController.text.trim());
      await prefs.setString('student_college', _collegeController.text.trim());
    } catch (e) {
      // Handle error silently
    }
  }
  ...
  Future<void> _generateMarksheet() async {
    ...
    try {
      ...
      // Load QR code images
      final playstoreQrBytes = await rootBundle.load(
        'assets/qr/playstore_qr.png',
      );
      final playstoreQrImage = pw.MemoryImage(
        playstoreQrBytes.buffer.asUint8List(),
      );

      final telegramQrBytes = await rootBundle.load(
        'assets/qr/telegram_qr.jpg',
      );
      final telegramQrImage = pw.MemoryImage(
        telegramQrBytes.buffer.asUint8List(),
      );
        ...
        pw.MultiPage(
          ...
          build: (pw.Context context) {
            return [
              ...
              _buildPdfFooter(playstoreQrImage, telegramQrImage),
            ];
          },
        ),
      );
  ...
  pw.Widget _buildPdfFooter(
    pw.ImageProvider playstoreQr,
    pw.ImageProvider telegramQr,
  ) {
    return pw.Column(
      children: [
        pw.Divider(),
